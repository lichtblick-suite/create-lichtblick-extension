...
function ExamplePanel({ context }: { context: PanelExtensionContext }): ReactElement {
  ...
  const [globalVariables, setGlobalVariables] = useState<undefined | Immutable<Map<string, any>>>();
  ...
  useLayoutEffect(() => {
    ...
    context.onRender = (renderState, done) => {
      ...
      setGlobalVariables(renderState.variables);
    };
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <h2>This is my global variables getter!</h2>
      ...
      <div style={{ marginTop: "1rem" }}>
        ...
        {globalVariables && globalVariables.size > 0 ? (
          <ul>
            {Array.from(globalVariables.entries()).map(([key, value]) => (
              <li key={key}>
          ...
          <div>No global variables set</div>
        )}
      </div>
